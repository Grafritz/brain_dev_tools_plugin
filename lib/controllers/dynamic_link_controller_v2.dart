  ...
  Future<void> _handleInitialUri() async {
    ...
    if (!_initialUriIsHandled) {
      ...
      try {
        ...
        //   logCat(
        //       '_handleInitialUri: _handleInitialUri: got uri: ${uri.path} ${uri.queryParametersAll}');
        // }
        //if (!mounted) return;
      ...
      if (mounted) {
        ...
        if (response.statusCode == 200) {
          try {
            ...
            showDialogProgress(show: false);
            Tools.showAlertDialog(msg: '${tr('msg_TraitementImpossible')}');
            logError(ex, trace: trace,
        ...
        showDialogProgress(show: false);
        Tools.showAlertDialog(msg: 'mounted:$mounted');
        throw TextEmptyException(tr('msg_TraitementImpossible'));
      }
    } on SocketException {
      setConnectivity();
      showDialogProgress(show: false);
      Tools.showAlertDialog(msg: tr('msg_No_Internet_connection'));
...
//endregion
  ...
  Future<void> initPlatformStateForUriUniLinks() async {
    ...
    try {
      ...
      // logCat(
      //     'initial uri: ${_initialUri?.path} | ${_initialUri?.queryParametersAll}');
  ...
  getNavigatorPushNamed2() {
    ...
    if (_deepLinks?.latestUri != null) {
      Navigator.of(Get.context!)
          .pushNamed(_deepLinks!.latestUriPath!, arguments: _deepLinks);
    }
  }
//endregion
}
