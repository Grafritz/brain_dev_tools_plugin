...
String convertFormatDate(
    {String? formatToSave, String? formatToDisplay, String? valueDate}) {
  if (valueDate == null || valueDate == '') {
    return '';
  }
  var _formatToSave = DateFormat(formatToSave, 'fr');
  var _formatToDisplay = DateFormat(formatToDisplay, 'fr');
  return _formatToDisplay.format(_formatToSave.parse(valueDate));
}

bool isUrl(String url) {
  return Uri.parse(url).host.isNotEmpty;
}
Future<bool> isAssetFileExists(String filePath) async {
  try {
    // Essayer de charger le fichier
    await rootBundle.load(filePath);
    return true; // Le fichier existe
  } catch (e) {
    return false; // Le fichier n'existe pas ou une erreur est survenue
  }
}
