...
class ColorResources {
  ...
  static setColorIsDark({Color? colorIsDark, Color? colorIsNotDark}) {
    return (Get.find<ThemeController>().isBrightnessDark) ? colorIsDark : colorIsNotDark;
  }

  static Color getPrimaryColor() {
    return Get.find<ThemeController>().isBrightnessDark ? const Color(0xFF002931) : const Color(0xFF003E47);
  }

  static Color getPrimaryTextColor() {
    return Get.find<ThemeController>().isBrightnessDark ? const Color(0xFF8dbac3) : const Color(0xFF003E47);
  }

  static Color getSecondaryHeaderColor() {
    return Get.find<ThemeController>().isBrightnessDark ? const Color(0xFFaaa818) : const Color(0xFFCFEC7E);
  }

  static Color? getColor({dynamic color = Colors.white}) {
    return (!Get.find<ThemeController>().isBrightnessDark) ? color : null;
  }

  static Color? getColorOrSetPrimaryColorDark({Color? color}) {
    return (!Get.find<ThemeController>().isBrightnessDark)
        ? (color != null)
            ? color
  ...
  static Color? getColorOrSetPrimaryColor({Color? color}) {
    return (!Get.find<ThemeController>().isBrightnessDark)
        ? (color != null)
            ? color
  ...
  static Color? getPrimaryColorDarkColor() {
    return !Get.find<ThemeController>().isBrightnessDark ? Theme.of(Get.context!).primaryColorDark : null;
  }

  static Color getPrimaryDarkOrWhiteColor() {
    return Get.find<ThemeController>().isBrightnessDark
        ? Colors.white
        : Theme.of(Get.context!).primaryColorDark;
  }

  static Color ccgColor = Colors.deepOrange;
  static Color chantDesperanceColor = Colors.indigo;
  static Color chantDesperanceLegacyColor = chantDesperanceColor;//Colors.red;
  static Color secondaryColor = const Color(0xFFE0EC53);

  static Color getColorAppOrYours({dynamic color = Colors.white}){
    return (Get.find<ThemeController>().isBrightnessDark) ? color : Theme.of(Get.context!).primaryColorDark;//getDefaultColorApp();
  }
  static Color getDefaultColorApp() {
    switch(EnvironmentVariable.environmentType) {
      case EnumEnvironmentType.chantdesperance:
        return chantDesperanceColor;
      case EnumEnvironmentType.chantdesperancelegacy:
        return chantDesperanceLegacyColor;
  ...
  static Color? getColorByApp({dynamic color = Colors.white}) {
    switch(EnvironmentVariable.environmentType) {
      case EnumEnvironmentType.chantdesperance:
        return (!Get.find<ThemeController>().isBrightnessDark) ? ColorResources.chantDesperanceColor : null;
      case EnumEnvironmentType.chantdesperancelegacy:
        return (!Get.find<ThemeController>().isBrightnessDark) ? ColorResources.chantDesperanceLegacyColor : null;
      case EnumEnvironmentType.chantchoraleetgroupe:
      case EnumEnvironmentType.chantlyric:
      case EnumEnvironmentType.lyricevangelique:
        return (!Get.find<ThemeController>().isBrightnessDark) ? color : null;
  ...
  static Color getGreyBaseGray1() {
    return Get.find<ThemeController>().isBrightnessDark ? const Color(0xFFd3d3d8) : const Color(0xFF8E8E93);
  }

  static Color getLightGray() {
    return Get.find<ThemeController>().isBrightnessDark ? const Color(0x00dbdbdb) : const Color(0xFFF3F3F3);
  }
  ...
  static Color primaryColor() =>
      Theme.of(Get.context!).primaryColor; //Color(0xFF003E47);
